...
class CatalogueRequest extends FormRequest
{
    ...
    {
        $id = $this->catalogue ? ',' . $this->catalogue->id : '';
        $required = $this->catalogue ? '' : 'required';

        return [
            'photo'      => [$required,'mimes:jpeg,jpg,png,svg'],
            'pdf'      => [$required,'mimes:pdf'],
            'name'      => 'required|max:255',
            'slug'      => [$required,'unique:catalogues,slug'. $id,'regex:/^[a-zA-Z0-9-]+$/'],
        ];
    }
    ...
    public function messages()
    {
        return [
            ...
            'pdf.mimes'  => __('PDF file format not supported.'),
