					...
					<div class="row justify-content-center">
						<div class="col-lg-12">
                            <form class="admin-form" action="{{ route('back.code.update',$code->id) }}"
                                ...
                                <div class="form-group">
                                    <label for="name">{{ __('Set Banner') }} *</label>
                                    <br>
                                        <img class="admin-img" src="{{ $code->banner ? asset('assets/images/'.$code->banner) : asset('assets/images/placeholder.png') }}"
                                            alt="No Image Found">
                                    <br>
                                    <span class="mt-1">{{ __('Banner Size Should Be 1160 x 585.') }}</span>
                                </div>
                                <div class="form-group position-relative">
                                    <label class="file">
                                        <input type="file"  accept="image/*"  class="upload-photo" name="banner" id="file"
                                            aria-label="File browser example">
                                        <span class="file-custom text-left">{{ __('Upload Banner...') }}</span>
                                    </label>
                                </div>
